import { Drawer, IconButton, Button, TextField, InputAdornment } from "@mui/material";
...
import { CourseTree, SearchResults } from "./CourseTree";
...
export function Sidebar({ catalog, mobileOpen, onClose }) {
  ...
  const content = <div className="sidebar-content">
    ...
    <div className="course-tree">
      {query
        ? <SearchResults catalog={catalog} store={store} navigate={navigate} onClose={onClose} query={query} />
        : <CourseTree catalog={catalog} store={store} navigate={navigate} onClose={onClose} />}
    </div>
  ...
  return <><aside className="sidebar desktop-sidebar">{content}</aside><Drawer open={mobileOpen} onClose={onClose} className="mobile-sidebar" PaperProps={{ className: "!w-[304px]" }}>{content}</Drawer></>;
}
