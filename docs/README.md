...
2. JupyterLite Notebook 负责编辑、执行、输出和本地保存；
3. 两层共享设计令牌，通过正式 Bridge 和主题扩展通信，不互相操作 DOM。

## 核心约束

- 保留 `notebooks/` 中的全部课程内容；
- 不引入数据库、账号系统和远程 Python 服务；
- 必须使用 React、TypeScript、Tailwind CSS、MUI Core、MUI X、React Bits 和 Zustand；
- Notebook 使用 JupyterLite 与 Pyodide 在浏览器中运行；
