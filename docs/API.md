...
> **课程内容映射说明（2026-09-06）**：当前课程采用 127 个课程资源（119 个连续展示章节资源 + 8 个模块大作业资源）。本文中的 assignments/tasks/practice 接口是未来服务化接入的兼容契约；默认不代表当前前端已经启用章节作业或独立实训。若实现提交流程，应优先使用 `capstone`/模块大作业语义，并在接口版本或资源类型中明确区分。
...
开发服务默认账号密码由 `AUTH_*_PASSWORD` 环境变量控制。账号与会话持久化于 `PDS_DATA_DIR`（默认 `server-data/`）JSON 文件，密码使用 argon2id 哈希存储，会话默认 24 小时过期；生产部署仍建议迁移到数据库并接入真实邮件发送。
...
管理员接口必须从服务端会话确认 `school_admin` 角色，并写入审计事件：创建者、用途、有效期、最大次数、撤销原因和请求 ID。服务端已使用 argon2id 密码哈希与 JSON 文件持久化；数据库唯一约束、审计表和密钥轮换仍待生产化。
