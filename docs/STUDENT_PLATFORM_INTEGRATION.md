...
> **当前课程状态（2026-08-02）**：当前主学习路径是章节学习 + 模块末大作业，不再以独立实训任务或章节作业作为教学主结构。本文件中的 assignments/tasks 接口属于未来平台接入预留能力；若启用，应将任务类型映射为模块大作业提交，而不是恢复旧的逐章实训路径。
...
- 获取当前登录学生的最小资料；
- 获取本人可参与的模块大作业或教师布置的扩展任务；
- 创建模块大作业尝试并进入隔离 Notebook；
- 提交 Notebook、关键结果和自检/评阅摘要；
- 同步课程进度与任务状态；
